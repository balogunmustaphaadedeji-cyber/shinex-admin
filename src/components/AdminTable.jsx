...
import { Skeleton, EmptyState, ErrorState, money } from "./ui";
...
/* Generic admin table for resources with simple GET (list) + optional DELETE.
   Hits the existing backend's /admin/:resource routes — nothing invented. */
export default function AdminTable({ resource, columns, title, searchable, deletable, renderActions }) {
    ...
    <div>
      ...
      {status === "ready" && rows.length > 0 && (
        <div className="flex items-center justify-between mt-4">
          <button disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="flex items-center gap-1 text-sm font-medium disabled:opacity-40" style={{ color: "#5B3FC6" }}>
            <ChevronLeft size={16} /> Previous
          </button>
          <span className="text-sm text-gray-400">Page {page}</span>
          <button disabled={rows.length < pageSize} onClick={() => setPage((p) => p + 1)} className="flex items-center gap-1 text-sm font-medium disabled:opacity-40" style={{ color: "#5B3FC6" }}>
