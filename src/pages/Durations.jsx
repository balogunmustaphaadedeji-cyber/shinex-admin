    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-5 flex flex-wrap items-end gap-3">
        <div className="w-28">
          ...
          <input type="number" value={newDuration.duration_days} onChange={(e) => setNewDuration((d) => ({ ...d, duration_days: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#14532D]/20" />
        </div>
        <div className="w-32">
          <span className="block text-xs font-medium text-gray-500 mb-1">Price (₦)</span>
          <input type="number" value={newDuration.price} onChange={(e) => setNewDuration((d) => ({ ...d, price: e.target.value }))} className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#14532D]/20" />
        </div>
          ...
          {durations.map((d) => (
            <div key={d.id} className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-1.5">
                <input type="number" value={d.duration_days} onChange={(e) => updateField(d.id, "duration_days", e.target.value)} className="w-16 rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-[#14532D]/20" />
                <span className="text-xs text-gray-500">days</span>
              </div>
              <div className="relative w-32">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">₦</span>
                <input type="number" value={d.price} onChange={(e) => updateField(d.id, "price", e.target.value)} className="w-full rounded-lg border border-gray-200 pl-6 pr-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-[#14532D]/20" />
              </div>
